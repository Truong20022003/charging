...
public class LowService extends Service {
    ...
    private void showNotification() {
        ...
        Intent mMainIntent = new Intent(getBaseContext(), LowReceiver.class);
        mMainIntent.setFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP | Intent.FLAG_ACTIVITY_NEW_TASK);

        PendingIntent mMainPendingIntent = PendingIntent.getBroadcast(getApplicationContext(), 2000, mMainIntent, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);

        RemoteViews contentView = new RemoteViews(getApplicationContext().getPackageName(), R.layout.service_full_batter);
        contentView.setTextViewText(R.id.tvTitle, getString(R.string.battery_remaining_less_than_20_tap_to_stop_ringtone));
    ...
    private String createNotificationChannel(NotificationManager notificationManager){
        String channelId = "Low";
        String channelName = "Charging service low";
